...
```bash
# Encode
curl "https://api.uwuapps.org/convert/encode?alg=base64&text=Hello&mode=encode"

# Decode
curl "https://api.uwuapps.org/convert/encode?alg=base64&text=SGVsbG8%3D&mode=decode"
```
...
```bash
curl "https://api.uwuapps.org/convert/encode?alg=base32&text=Hello&mode=encode"
```
...
```bash
curl "https://api.uwuapps.org/convert/encode?alg=ascii85&text=Hello&mode=encode"
```
...
curl "https://api.uwuapps.org/convert/encode?alg=urlenc&text=Hello+World&mode=encode"
```
...
curl "https://api.uwuapps.org/convert/encode?alg=codepoints&text=Hi&mode=encode"
```
...
```bash
curl "https://api.uwuapps.org/convert/encode?alg=baudot&text=HELLO&mode=encode"
```
...
```bash
# Title case
curl "https://api.uwuapps.org/convert/encode?alg=case&text=hello+world&style=title"
```
...
```bash
curl "https://api.uwuapps.org/convert/encode?alg=reverse&text=Hello"
```
...
```bash
curl "https://api.uwuapps.org/convert/encode?alg=replace&text=Hello+World&find=World&repl=API"
```
